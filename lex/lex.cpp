...
{
	//TODO : 龙书上的词法分析器双缓冲技术
	if (pointer - start == LEX_BUFFER_SIZE && type == GET)
	{	/* all context in buffer has been proceed, re-fill the buffer*/
		//fillBuffer();
		//pointer = buffer;
	}
	switch(type)
	...
	case '!':
		{
			...
			switch(peek)
			{
			case '=':	op = OP_NE;	goto length_2;
			default:	op = OP_LOGNOT;
				getNextChar(BACK);
				goto length_1;
			}
		}
	...
	case '%':
		{
			...
			switch(peek)
			{
			case '=':	op = OP_ALMOD;	goto length_2;
			default:	op = OP_MOD;
				getNextChar(BACK);
				goto length_1;
			}
	...
	case '^':
		{
			...
			switch(peek)
			{
			case '=':	op = OP_ALBITXOR;	goto length_2;
			default:	op = OP_BITXOR;
				getNextChar(BACK);
				goto length_1;
			}
		}
	case '&':
		{
			...
			switch(peek)
			{
			case '&':	op = OP_LOGAND;		goto length_2;
			case '=':	op = OP_ALBITAND;	goto length_2;
			default:	op = OP_BITAND;
				getNextChar(BACK);
				goto length_1;
			}
		}
	case '*':
		{
			...
			switch(peek)
			{
			case '=':	op = OP_ALMULT;	goto length_2;
			default:	op = OP_MULT;
				getNextChar(BACK);
				goto length_1;
			}
	...
	case '+':
		{
			...
			switch(peek)
			{
			case '=':	op = OP_ALPLUS;	goto length_2;
			case '+':	op = OP_INC;	goto length_2;
			default:	op = OP_PLUS;
				getNextChar(BACK);
				goto length_1;
			}
	...
	case '-':
		{
			...
			switch(peek)
			{
			case '=':	op = OP_ALMINUS;	goto length_2;
			case '-':	op = OP_DEC;		goto length_2;
			case '>':	op = OP_POINTER;	goto length_2;
			default:	op = OP_MINUS;
				getNextChar(BACK);
				goto length_1;
			}
	...
	case '|':
		{
			...
			switch(peek)
			{
			case '=':	op = OP_ALBITOR;	goto length_2;
			case '|':	op = OP_LOGOR;		goto length_2;
			default:	op = OP_BITOR;
				getNextChar(BACK);
				goto length_1;
			}
	...
	*tp = '\0';
	...
	else
	{
		currentToken->token_type =
			currentToken->token_value.numVal->type= T_INT_CON;
...
int Lex::getNextToken(void)
{
	for (;;){
		...
		case 'i': case 'l': case 'r': case 's': case 't' :case 'u': case 'v':
		case 'w':
			return parseLetter(tmpBuffer[0]);
		case 'A': case 'B': case 'C': case 'E': case 'D': case 'F': case 'G':
		case 'H': case 'I': case 'J': case 'K': case 'L': case 'M': case 'N':
		case 'O': case 'P': case 'Q': case 'R': case 'S': case 'T': case 'U':
		case 'V': case 'W': case 'X': case 'Y': case 'Z': case 'h': case 'j':
		case 'k': case 'm': case 'n': case 'o': case 'p': case 'q': case 'x':
...
NS_LEX_PODS::Token::Token(Token &tok ) 
