...
Type::Type( TYPE_OPERATOR _oper, Type* _baseType,int _arrEls, Type* _list, 
	   int _size, int _align,  void* _symbol)
{
	this->sym = NULL;
	this->name = NULL;
	type_op = _oper;
...
paraPrint:
			...
			{
				return buffer + sprintf(buffer,"%s",
					to2c[type_op]);
			}
	}
	return buffer;
