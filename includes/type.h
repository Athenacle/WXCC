...
{
public:
	TypeException(int tp = 0, const char* st = "no exception.")
		:Exception(tp, st){}
...
class Type : public WXCC
{
	...
	{
		return (ty.type_op & BASE_TYPE_MASK) == BASE_TYPE_MASK ||
			ty.type_op == TO_CHAR;
	}
...
public:
	...
	explicit Type(NS_TYPE_OP::TYPE_OPERATOR = NS_TYPE_OP::TO_VOID, Type* = NULL,
			int = 0,  Type* = NULL, int = 4, int = 4, void* = NULL);
	...
	{
		assert(this->type_op == TO_FUNCPARA || 
			this->type_op == TO_FUNCTION);
