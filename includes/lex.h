...
namespace NS_LEX_CONSTANTS{
	...
	*/
	...
	const unsigned long TYPE_SSQ
		= (0x00021000|0x00020100|0x00020010);
...
namespace NS_LEX_PODS{
	...
	struct Position
	{
		...
		Position(const Position& pos)
			:filename(pos.filename), line(pos.line), place(pos.place)
		{ }
		Position(const char *file, int l = 1, int p = 0)
	...
	public:
		...
		union token_value
		{
			...
			const char 	*id_name;
			const char	*string;
			NS_LEX_CONSTANTS::KEYWORD	keyword;
			NS_LEX_CONSTANTS::OP	op;
		}token_value;

		bool inline operator == (NS_LEX_CONSTANTS::TYPE ty)
		{
			return token_type == ty;
		}


		bool inline operator == (NS_LEX_CONSTANTS::OP _op)
		{
			return token_type == T_OPERATOR && token_value.op == _op;
		}

		bool inline operator == (NS_LEX_CONSTANTS::KEYWORD _key)
		{
			return token_type == T_KEY && token_value.keyword == _key;
		}
		...
		explicit Token(NS_LEX_CONSTANTS::KEYWORD );
		explicit Token(NS_LEX_CONSTANTS::TYPE, const char*);
		explicit Token(Number *);
		explicit Token(Token &tok);
		Token()
		{
			token_type = NS_LEX_CONSTANTS::T_NONE;
			token_pos = NULL;
			token_value.id_name = NULL;
		}

	};
};//namespace NS_LEX_PODS

namespace NS_LEX_TOOLS
{
	using NS_LEX_PODS::Token;
	using namespace NS_LEX_CONSTANTS;
	...
	{
		return _tok.token_type == T_INT_CON || 
			_tok.token_type == T_FLOAT_CON ||
	...
	bool inline isBaseType(const Token &_tok)
	{
		return is(_tok, T_KEY) && (
			(_tok.token_value.keyword & TYPE_SPECIFIER_MASK)
			== TYPE_SPECIFIER_MASK || matchKEY(_tok, KEY_TVOID) 
				|| matchKEY(_tok, KEY_KVOID) );
	}
	...
	{
		return is(_tok, T_KEY) && 
			(_tok.token_value.keyword == KEY_FOR ||
			_tok.token_value.keyword == KEY_WHILE ||
	...
	{
		return isOP(_tok) && 
			(_tok.token_value.op & OP_ASGN_MASK) == OP_ASGN_MASK;
	}
...
class Lex
{
	...
	friend NS_LEX_PODS::Token::Token(NS_LEX_CONSTANTS::KEYWORD );
	friend NS_LEX_PODS::Token::Token(NS_LEX_CONSTANTS::TYPE, const char*);
	friend NS_LEX_PODS::Token::Token(NS_LEX_PODS::Number *);
	friend NS_LEX_PODS::Token::Token(Token &tok ) ;
private:
	Lex(const Lex&);
	Lex& operator = (const Lex&);

	static NS_LEX_PODS::Token *currentToken;
	static NS_LEX_PODS::Position *currentPos;

	NS_LEX_PODS::Token *newTok;
	...
	NS_LEX_PODS::Number* tryParseFloatNumber(char*, NS_LEX_PODS::Number **);
	NS_LEX_PODS::Number* tryParseDecNumber(char*);
	NS_LEX_PODS::Number* tryParseHexNumber(char*);
	NS_LEX_PODS::Number* tryParseOctNumber(char*);
	int newNumber(char *, int);
	int number(char);
	int newIdentifier(const char*, int);
	int identifier(char*);
	NS_LEX_CONSTANTS::KEYWORD letter(char);
...
#endif
	NS_LEX_PODS::Token* getToken(void)
	{
		return currentToken;
	}

	NS_LEX_PODS::Position* getPos(void)
	{
		return currentPos;
	}

	NS_LEX_PODS::Token* getLastToken(void)
	{
		return new NS_LEX_PODS::Token(*currentToken);
	}
