...
ST* Parser::c_parser_funtion_definition(Env &globalEnv, Identifier &id) const
{
	...
	fprintf(wxccErr, "Function %s Begin. Type: %s.--------------------- \n\n", 
		id.getIDname(), id.getType().getTYPEName());
	IR* blockBegin = new IR();
	Block* retIR = c_parser_compound_statement(fun);
	IR::print(blockBegin);
	fprintf(wxccErr, "\nFunction %s End.----------------------------------\n", 
		id.getIDname());
	need(OP_RIGHTBRACE);
		...
		if (blockIR != NULL)
		{
			...
			if (blockIR->continueList != NULL)
			{
				if (ret->continueList == NULL)
					...
					ret->continueList = IRList::mergeChain(*ret->continueList, *blockIR->continueList);
			}
		}
...
Block* Parser::c_parser_select_statement(Env& env) const
{
	...
	if (*cur == KEY_IF)
	{
		...
		if (*cur == KEY_ELSE)
		{
			...
			IRList::mergeChain(*falseStmt->nextList, 
				*IRList::makeIRList(*falseGoto));
			IRList *listBeforeElse = IRList::makeIRList(*irBefore);
			IRList::backPatch(*condExpr->getTree().falseList, *la);
			IRList *tmp = IRList::mergeChain(*falseStmt->nextList, 
					irBefore);
			stmt->nextList = IRList::mergeChain(*tmp, *falseStmt->nextList);

			if (falseStmt->breakList != NULL)
				if (stmt->breakList != NULL)
					stmt->breakList = IRList::mergeChain(*stmt->breakList
					, *falseStmt->breakList);
				else
					stmt->breakList = falseStmt->breakList;
			if (falseStmt->continueList != NULL)
				if (stmt->continueList != NULL)
					stmt->continueList = IRList::mergeChain(*stmt->continueList
					, *falseStmt->continueList);
				else
		...
		else
		{
			...
			if (trueStmt != NULL && trueStmt->nextList != NULL)
				stmt->nextList = IRList::mergeChain(*condExpr->getTree().falseList, 
				*trueStmt->nextList);
		}
	}
...
label:
	...
	{
		parserError(PAR_ERR_INT_STR, stmtErrors[PE_DECL_AFTER_STMT], 
			cur->token_pos->line);
meet_type:
 ...
 */
{
	IRList *condBeginList, *bodyBeginList, *retList, *incrList, *backList;
	Block *ret = new Block();
	ret->nextList = IRList::makeIRList(* new IR(""));
	need(OP_LEFTBRACK);
	next();
	...
	IR *stmtIR = new IR(*stmtLabel);
	Block *stmt = c_parser_statement(env);
	...
	backList = ret->nextList = IRList::mergeChain(*condExpr->nextList, *condExpr->getTree().trueList);
