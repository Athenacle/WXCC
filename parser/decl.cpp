...
meet_comma:
	...
	{
		if (declared != 1)
			parserError(PAR_ERR_INT, declError[DE_MORE_DECL_WITH_FUNC], 
				cur->token_pos->line);
		else
		{
...
Type* Parser::c_parser_parameter_type_list(void) const
{
	...
	if (matchOP(*cur, OP_RIGHTBRACK)) // no parameter. eg. void foo();
	{
		...
		parserWarning(PARER_WE_USE_STRING, declWarning[DW_LACK_PARA_LIST], 
			cur->token_pos->line);
	}
	else
	{
...
paraList:
			...
			if (retST != NULL)
			{
				retST->second = new Type(TO_FUNCPARA, retST->second, 
					0, parameterList, 4, 4, retST->first);
			...
			{
				parserError(PARER_WE_USE_STRING, 
					declError[ DE_NEED_COMMA], cur->token_pos->line);
			}
