...
void Env::newIdentifier(Identifier* id)
{
	...
	{
		Identifier * idret = 
			localID.existCurrentScope(id->getIDName());
		if (idret == NULL)
		{
		...
		{
			semError(USE_MY_STR_SSD, tableError[TE_ERR_REDECL], 
				id->getSymbolLine(), id->getIDName(), 
				(idret)->getSymbolLine());
		}
	}
	else
	{
		Identifier * funcRet = 
			localFunc.existCurrentScope(id->getIDName());
		if (funcRet == NULL)
		{
