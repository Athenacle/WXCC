...
if (CODE_COVERAGE AND NOT CODE_COVERAGE_ADDED)
    ...
    if (CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang" OR CMAKE_CXX_COMPILER_ID MATCHES
        ...
        if (${CMAKE_VERSION} VERSION_LESS "3.17.0")
            ...
            add_custom_target(
                ccov-clean COMMAND ${CMAKE_COMMAND} -E rm -f
                                   ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list
        ...
        # processing
        add_custom_target(ccov-libs COMMAND ; COMMENT "libs ready for coverage report.")
...
function (target_code_coverage TARGET_NAME)
    ...
    if (CODE_COVERAGE)
        ...
        if (CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang" OR CMAKE_CXX_COMPILER_ID MATCHES
                                                              "(Apple)?[Cc]lang")
            target_compile_options(${TARGET_NAME} ${TARGET_VISIBILITY} -fprofile-instr-generate
                                   -fcoverage-mapping)
            target_link_options(${TARGET_NAME} ${TARGET_VISIBILITY} -fprofile-instr-generate
                                -fcoverage-mapping)
        elseif (CMAKE_C_COMPILER_ID MATCHES "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES "GNU")
            ...
            if (CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang" OR CMAKE_CXX_COMPILER_ID MATCHES
                ...
                add_custom_target(
                    ...
                    COMMAND ${CMAKE_COMMAND} -E echo "-object=$<TARGET_FILE:${TARGET_NAME}>" >>
                            ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list DEPENDS ${TARGET_NAME})
...
function (add_code_coverage)
    if (CODE_COVERAGE)
        ...
        elseif (CMAKE_C_COMPILER_ID MATCHES "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES "GNU")
            add_compile_options(
                -fprofile-arcs -ftest-coverage $<$<COMPILE_LANGUAGE:CXX>:-fno-elide-constructors>
                -fno-default-inline)
            link_libraries(gcov)
        endif ()
    endif ()
...
function (add_code_coverage_all_targets)
    ...
    if (CODE_COVERAGE)
            ...
            if (WIN32)
                add_custom_target(
                    ...
                    COMMAND
                        ...
                        ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/profraw.list\; llvm-profdata.exe merge
                        -o ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/all-merged.profdata -sparse
                        $$FILELIST)
            else ()
                add_custom_target(
                    ccov-all-processing
        ...
        elseif (CMAKE_C_COMPILER_ID MATCHES "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES "GNU")
            ...
            if (${CMAKE_VERSION} VERSION_LESS "3.17.0")
                add_custom_target(
                    ccov-all-capture COMMAND ${CMAKE_COMMAND} -E remove -f ${COVERAGE_INFO}
                    COMMAND ${LCOV_PATH} --directory ${CMAKE_BINARY_DIR} --capture --output-file
                            ${COVERAGE_INFO} COMMAND ${EXCLUDE_COMMAND} DEPENDS ccov-all-processing)
            else ()
                add_custom_target(
                    ccov-all-capture COMMAND ${CMAKE_COMMAND} -E rm -f ${COVERAGE_INFO}
                    COMMAND ${LCOV_PATH} --directory ${CMAKE_BINARY_DIR} --capture --output-file
                            ${COVERAGE_INFO} COMMAND ${EXCLUDE_COMMAND} DEPENDS ccov-all-processing)
            endif ()
            ...
            add_custom_target(
                ccov-all COMMAND ${GENHTML_PATH} -o ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/all-merged
                                 ${COVERAGE_INFO} -p ${CMAKE_SOURCE_DIR} DEPENDS ccov-all-capture)
